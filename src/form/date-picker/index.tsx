...
import { DialogHelper, DialogOptions, DialogSize } from "../../providers/dialog-provider/types";
...
const DatePickerDialog = ({ label, mode, date, format = "YYYY-MM-DD", helper, validator }: IDatePickerDialog): any => {
    ...
    useEffect(() => {
        ...
        const modeIsSingle = [null, undefined, DatePickerMode.MONTH].includes(mode);
    ...
    const selectDate = (e: React.MouseEvent, year: number, month: number, day: number): void => {
        ...
        } else if (mode === DatePickerMode.MONTH) setDates([dateMoment.format(format)]);
        else setDates([dateMoment.format(format)]);
    ...
    const returnDate = (): void => {
        if (mode === DatePickerMode.MULTI) return helper.dismiss(dates.length > 0 ? dates : null);
        if (mode === DatePickerMode.MONTH) return helper.dismiss(dates[0]);
        return helper.dismiss(dates[0]);
    };
    ...
    return (
        <div className="react-simple-widget date-picker">
            <FieldDecoration
                ...
                trailing={trailing}
                error={error}
                stickyFloatingLabel={stickyFloatingLabel || !!date || dialogOpen}>
